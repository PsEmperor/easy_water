...
public class ApplyirrigatePreludeFragment extends Fragment implements
	...
	private TextView work_stress, filter_pressure, flow_rate, cumulative_water,
			cumulative_power;
	...
	public View onCreateView(LayoutInflater inflater, ViewGroup container,
		...
		actionBar = (MainActionBar) view.findViewById(R.id.actionbar_prelude);
		...
		bt_water_pump = (ToggleButton) view
				.findViewById(R.id.toggle_prelude_water_pump);
		bt_filter = (ToggleButton) view
				.findViewById(R.id.Toggle_prelude_filter);
		bt_fertilizer_drill = (ToggleButton) view
				.findViewById(R.id.Toggle_prelude_fertilizer_drill);
		work_stress = (TextView) view
				.findViewById(R.id.text_prelude_work_stress);
		filter_pressure = (TextView) view
				.findViewById(R.id.text_prelude_filter_pressure);
		flow_rate = (TextView) view.findViewById(R.id.text_prelude_flow_rate);
		cumulative_water = (TextView) view
				.findViewById(R.id.text_prelude_cumulative_water);
		cumulative_power = (TextView) view
				.findViewById(R.id.text_prelude_cumulative_power);
		...
		str1 = (String) SharedUtils.getParam(getActivity(), "FirstDerviceID",
				"");
	...
	private void init() {
		...
		// isWaterPump = (Boolean) SharedUtils.getParam(getActivity(),
		// "water_pump", false);
		// isFilter = (Boolean) SharedUtils.getParam(getActivity(), "filter",
		// false);
		// isFertilizer_drill = (Boolean) SharedUtils.getParam(getActivity(),
		// "fertilizer_drill", false);
	...
	public void onClick(View v) {
		...
		case R.id.toggle_prelude_water_pump:
			if(PermissionListBean.get(7).getOpertionStat().contains("4")){
				if (!NetStatusUtil.isNetValid(getActivity())) {
					Toast.makeText(getActivity(), "当前网络不可用！请检查您的网络状态！", Toast.LENGTH_SHORT)
							.show();
				} else {
					if (isWaterPump == false) {
						...
						try {
							...
							js_request1.put("pumpEquID", beens.get(0)
									.getPumpEquID());
						...
						RequestParams param3 = new RequestParams(
								URL.acquireIrriState + str1); // 网址(请替换成实际的网址)
								...
								new CommonCallback<String>() {
									...
									public void onSuccess(String arg0) {
										Toast.makeText(getActivity(), "请求成功",
												Toast.LENGTH_SHORT);
										Gson gson = new Gson();
										final IrriGroupStateBean fromJson = gson
												.fromJson(arg0,
														IrriGroupStateBean.class);
										progressDialog.dismiss();
										if ("1".equals(fromJson.getCode())) {
											...
											try {
												param3.setAsJsonContent(true);
												js_request1.put("firstDerviceID",
														str1);
												js_request1.put("pumpEquID", beens
														.get(0).getPumpEquID());
												js_request1.put("pumpSwitch", 0);
												js_request1.put("irriState",
														fromJson.getCode());
				...
				bt_water_pump.setChecked(false);
				bt_water_pump.setEnabled(false);
			...
			if (!NetStatusUtil.isNetValid(getActivity())) {
				...
				if (isFilter == false) {
					...
					try {
						param3.setAsJsonContent(true);
						js_request1.put("firstDerviceID", str1);
						js_request1.put("filterEquID", beens.get(0)
								.getFilterEquID());
